...
web3.eth.sendTransaction({from:eth.defatltAccount , 
    to:"销售商地址" , gas:30000 }).then(address=>{instance.setInflow.sendTransaction(address, 
        "药品批号", "流入日期", "交易量", "销售商地址", {from:eth.defatltAccount})}).then(web3.eth.sendTransaction({from:"上帝账户", to:eth.defatltAccount, gas:30000}));

info = instance.getProduce.call("药品批号");
var drugCode = info[1],
    tmpDrug = info[2],
    tmpState = info[3],
